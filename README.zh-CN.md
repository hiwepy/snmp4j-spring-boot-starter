...
**snmp4j-spring-boot-starter** 是一个面向 使用 snmp4j 的应用 的 Spring Boot Starter，用于将 **snmp4j** 集成到 Spring Boot 应用中。它提供自动装配、属性绑定与开箱即用的 Bean，使应用以最小配置即可使用 snmp4j 的全部能力。

| 维度 | 说明 |
|---|---|
| 类型 | Spring Boot Starter |
| 消费方 | 使用 snmp4j 的 Spring Boot 应用 |
| 核心能力 | 自动装配、属性绑定、开箱即用的 snmp4j Bean |
| JDK | `17` |
| 坐标 | `io.github.easy4j:snmp4j-spring-boot-starter:2.7.x.20260527-SNAPSHOT` |
| 配置前缀 | `snmp4j` |
...
| 能力 | 状态 | 说明 |
|---|:---:|---|
| 自动装配 | ✅ 稳定 | 自动注册 snmp4j 相关 Bean |
| 属性绑定 | ✅ 稳定 | 绑定 `snmp4j.*` 到 `PoiProperties` |
...
| 属性 | 类型 | 默认值 | 必填 | 说明 | 敏感 |
|---|---|---|:---:|---|:---:|
| `snmp4j.enabled` | boolean | `true` | 否 | 是否启用 Starter | 否 |
<!-- 更多属性见下方 -->
...
| 症状 | 诊断 | 解决 |
|---|---|---|
| Bean 未创建 | 查看自动装配报告 | 确认 `snmp4j.enabled=true` 与 classpath |
...
[返回顶部](#readme-top) · [问题反馈](https://github.com/easy-4-java/snmp4j-spring-boot-starter/issues) · [仓库地址](https://github.com/easy-4-java/snmp4j-spring-boot-starter)
